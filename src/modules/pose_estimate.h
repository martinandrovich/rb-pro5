...
constexpr auto START_VAL = 3.1347;
constexpr auto DELTA = 2.04086 - 2.01806;
...
public:
    pose_estimate_t(cv::String filepath, float scalefactor_) : scalefactor(scalefactor_), integral_mov(0), integral_dir(0)
    {
        ...
        else
        {
            ...
            // scale the image
            int scaleX = ((float)(img.cols * IMG_SCALE) / scalefactor);
            int scaleY = ((float)(img.rows * IMG_SCALE) / scalefactor);
            ...
            /*
            cv::Point marble(xaxis / 2 + 18.217300 * IMG_SCALE, yaxis / 2 - 0.878297 * IMG_SCALE );
            cv::circle(img_scaled, marble, 10, cv::Scalar(0, 0, 255) );
            marble = cv::Point(xaxis / 2 + 23.752800 * IMG_SCALE, yaxis / 2 - 0.59 * IMG_SCALE );
            cv::circle(img_scaled, marble, 10, cv::Scalar(0, 0, 255)     );
            marble = cv::Point(xaxis / 2 + 23.264400 * IMG_SCALE, yaxis / 2 - 18.303200 * IMG_SCALE );
            cv::circle(img_scaled, marble, 10, cv::Scalar(0, 0, 255) );
            */
    ...
    */
    {
        ...
        for (auto &temp_ : temp)
        {
            temp_.dist = temp_.dist * IMG_SCALE;
            ray_data[i] = temp_;
            i++;
        }
    }
    ...
    */
    {
        ...
        for (size_t i = 0; i < PARTICLES; i++)
        {
            ...
            particles[i].pos.x += integral_mov * cos(particles[i].orient.yaw) * IMG_SCALE + distx(gen);
            particles[i].pos.y += integral_mov * sin(particles[i].orient.yaw) * IMG_SCALE + disty(gen);
        ...
        std::pair<size_t, size_t> particlerange[THREADS];
        int temp = 0;

        for (size_t i = 0; i < THREADS; i++)
        {
            if (i == THREADS - 1)
                particlerange[i] = std::pair<int, int>(temp, PARTICLES);
            else
                particlerange[i] = std::pair<int, int>(temp, temp + PARTICLES / THREADS);
            temp += PARTICLES / THREADS + 1;
    ...
    */
    {
        ...
        for (i = par_start; i < par_end; ++i)
        {
            ...
            auto it = std::find_if(angle.begin(), angle.end(),
                                   [&](const auto &angle) { return std::abs(angle - particles[i].orient.yaw) <= DELTA; });
    ...
    */
    {
        ...
        int index = PARTICLES * distribution(generator);

        float beta = 0;

        //get max weight
        float *weights_max = std::max_element(weights, weights + PARTICLES);
    ...
    */
    {
        ...
        // init tripple vector
        lookuptable_mean = std::vector<std::vector<std::vector<float>>>(img_scaled.rows, std::vector<std::vector<float>>(img_scaled.cols, std::vector<float>(ANGLES, 0)));
        ...
        std::vector<std::future<void>> t;
        std::pair<size_t, size_t> yrange[THREADS];
        int temp = 0;
        for (size_t i = 0; i < THREADS; i++)
        {
            if (i == THREADS - 1)
                ...
                yrange[i] = std::pair<int, int>(temp, temp + img_scaled.rows / THREADS);
            temp += img_scaled.rows / THREADS + 1;
            ...
            for (size_t x = 0; x < (size_t)img_scaled.cols; ++x)
            {
                ...
                else
                {
                    for (size_t i = 0; i < dirs.size(); i++)
                    {
                        ...
                        stop = cv::Point2f(x + LIDAR_SCALE * dirs[i].x, y + LIDAR_SCALE * dirs[i].y);
    ...
    */
    {
        ...
        mean_position.pos.x         /= PARTICLES;
        mean_position.pos.y         /= PARTICLES;
        mean_position.orient.yaw    /= PARTICLES;
...
private:
    ...
    */
    ...
    pose_t particles[PARTICLES];
    pose_t new_particles[PARTICLES];
    pose_t mean_position;
    float weights[PARTICLES];
